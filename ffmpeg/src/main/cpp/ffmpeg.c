...
void sub2video_update(InputStream *ist, AVSubtitle *sub) {
    ...
    for (i = 0; i < num_rects; i++)
        sub2video_copy_rect((uint8_t *) dst, dst_linesize, frame->width, frame->height,
                            sub->rects[i]);
...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time,
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (!vid && enc->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            //回掉进度
            int nb_frames = (int) input_files[0]->ctx->streams[0]->nb_frames;
            onChange(frame_number, nb_frames);
 ...
 */
static int transcode(void (*onChange)(int current, int target)) {
    int ret, i;
    AVFormatContext *os;
    OutputStream *ost;
    InputStream *ist;
    ...
    while (!received_sigterm) {
        ...
        print_report(0, timer_start, cur_time, onChange);
    }
#if HAVE_THREADS
    free_input_threads();
#endif
    ...
    print_report(1, timer_start, av_gettime_relative(), onChange);
...
int ffmpeg_exec(int argc, char **argv, void (*onChange)(int current, int target)) {
    ...
    if (ret < 0)
        main_return_code = exit_program(1);
...
//     }
    ...
    if ((decode_error_stat[0] + decode_error_stat[1]) * max_error_rate < decode_error_stat[1])
        main_return_code = exit_program(69);

    main_return_code = exit_program(received_nb_signals ? 255 : main_return_code);
    ffmpeg_cleanup(0);
