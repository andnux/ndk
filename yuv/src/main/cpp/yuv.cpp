...
#define YUV_UTILS_JAVA "top/andnux/yuv/YuvManager"
...
int Jni_I420ToNV21(JNIEnv * env,jclass clazz,jbyteArray yuv420p,jbyteArray yuv420sp,jint width,jint height,jboolean swapUV){
    ...
    int ret=libyuv::I420ToNV21((const uint8_t *) yuv420pData, width,
                               (const uint8_t *) (yuv420pData + ySize + stride[swapUV]), width >> 1,
                               (const uint8_t *) (yuv420pData + ySize + stride[1-swapUV]), width >> 1,
                               (uint8_t *) yuv420spData, width, (uint8_t *) (yuv420spData + ySize), width, width, height);
...
int Jni_NV21ToI420(JNIEnv * env,jclass clazz,jbyteArray yuv420p,jbyteArray yuv420sp,jint width,jint height,jboolean swapUV){
    ...
    jbyte * yuv420pData=env->GetByteArrayElements(yuv420p,JNI_FALSE);
    jbyte * yuv420spData=env->GetByteArrayElements(yuv420sp,JNI_FALSE);
    int ret=libyuv::NV21ToI420((const uint8_t *) yuv420spData, width, (const uint8_t *) (yuv420spData + ySize), width,
                               (uint8_t *) yuv420pData, width,
                               (uint8_t *) (yuv420pData + ySize + stride[swapUV]), width >> 1,
                               (uint8_t *) (yuv420pData + ySize + stride[1-swapUV]), width >> 1, width, height);
    env->ReleaseByteArrayElements(yuv420p,yuv420pData,JNI_OK);
    env->ReleaseByteArrayElements(yuv420sp,yuv420spData,JNI_OK);
    return ret;
}
...
void Jni_NV21Scale(JNIEnv * env,jclass clazz,jbyteArray src,jint width,jint height,jbyteArray dst,jint dst_width,jint dst_height,int mode){
    ...
    libyuv::ScalePlane((const uint8_t *) srcData, width, width, height,
                       (uint8_t *) dstData, dst_width, dst_width, dst_height,(libyuv::FilterMode) mode);
...
void Jni_I420Scale(JNIEnv * env,jclass clazz,jbyteArray src,jint width,jint height,jbyteArray dst,jint dst_width,jint dst_height,int mode,jboolean swapUV){
    ...
    jbyte * srcData=env->GetByteArrayElements(src,JNI_FALSE);
    jbyte * dstData=env->GetByteArrayElements(dst,JNI_FALSE);
    libyuv::I420Scale((const uint8_t *) srcData, width, (const uint8_t *) (srcData + ySize), width >> 1,
                      (uint8_t *)(srcData + ySize + (ySize >> 2)), width >> 1, width, height,
                      (uint8_t *) dstData, dst_width, (uint8_t *) (dstData + dstYSize+swap[swapUV]), dst_width >> 1,
...
void Jni_NV21ToI420Rotate(JNIEnv * env,jclass clazz,jbyteArray src,jint width,jint height,jbyteArray dst,jint de,jboolean swapUV){
    ...
    jbyte * srcData=env->GetByteArrayElements(src,JNI_FALSE);
    jbyte * dstData=env->GetByteArrayElements(dst,JNI_FALSE);
    rotatePlaneFunc[de]((const uint8_t *) srcData, width, (uint8_t *) dstData, dst_stride[de], width, height);
    rotateUVFunc[de]((const uint8_t *) (srcData+ySize), width, (uint8_t *)(dstData+ySize+swap[swapUV]), dst_stride[de]>>1,
                     (uint8_t*)(dstData+ySize+swap[1-swapUV]),dst_stride[de]>>1,width>>1, height>>1);
    env->ReleaseByteArrayElements(src,srcData,JNI_OK);
    env->ReleaseByteArrayElements(dst,dstData,JNI_OK);
}
